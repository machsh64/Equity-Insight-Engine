...
export default function EditQuarterModal({ quarter, onClose, onSuccess }: Props) {
  const [formData, setFormData] = useState({
    quarter: quarter.quarter,
    pe: quarter.pe?.toString() || '',
    pb: quarter.pb?.toString() || '',
    ps: quarter.ps?.toString() || '',
    roe: quarter.roe?.toString() || '',
    roic: quarter.roic?.toString() || '',
    wacc: quarter.wacc?.toString() || '',
    revenue_yoy: quarter.revenue_yoy?.toString() || '',
    gross_margin: quarter.gross_margin?.toString() || '',
    fcf_margin: quarter.fcf_margin?.toString() || '',
    capex_ratio: quarter.capex_ratio?.toString() || '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await quarterApi.update(quarter.id, data);
      onSuccess();
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl my-8 max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold mb-4">编辑季度数据</h2>
